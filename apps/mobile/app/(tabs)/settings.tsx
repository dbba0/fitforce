...
function SettingsRow({ icon, label, value, onPress, rightEl, color }: {
  ...
  const content = (
    <View style={[styles.settingsRow, { borderBottomColor: theme.border }]}>
      ...
      <Text style={[styles.settingsLabel, { color: theme.text, fontFamily: "Outfit_500Medium" }]}>{label}</Text>
      <View style={styles.settingsRight}>
        {value ? (
          <Text style={[styles.settingsValue, { color: theme.textSecondary, fontFamily: "Outfit_400Regular" }]}>{value}</Text>
        ) : null}
    ...
    <View style={[styles.container, { backgroundColor: theme.background }]}>
      ...
      >
        ...
        <Animated.View entering={FadeInDown.delay(80).duration(400)}>
          <Text style={[styles.sectionLabel, { color: theme.textSecondary, fontFamily: "Outfit_600SemiBold" }]}>
            {t("language")}
          </Text>
        ...
        <Animated.View entering={FadeInDown.delay(140).duration(400)}>
          ...
          <View style={[styles.card, { backgroundColor: theme.card }]}>
            <SettingsRow
              ...
              color="#6C63FF"
              rightEl={
                <View style={[styles.themeBadge, { backgroundColor: isDark ? "#6C63FF20" : "#FFB54720" }]}>
                  <Ionicons name={isDark ? "moon" : "sunny"} size={14} color={isDark ? "#6C63FF" : "#FFB547"} />
                </View>
              }
            />
          </View>
        ...
        <Animated.View entering={FadeInDown.delay(380).duration(400)} style={styles.appInfo}>
          <Text style={[styles.appName, { color: theme.accent, fontFamily: "Outfit_800ExtraBold" }]}>FitForce</Text>
          <Text style={[styles.appTagline, { color: theme.textSecondary, fontFamily: "Outfit_400Regular" }]}>
            {t("startYourJourney")}
          </Text>
        </Animated.View>
        ...
        {isAuthenticated && (
          <Animated.View entering={FadeInDown.delay(440).duration(400)} style={{ paddingBottom: 20 }}>
            ...
            >
              ...
              <Text style={[styles.logoutBtnText, { fontFamily: "Outfit_600SemiBold" }]}>
                {t("logout")}
              </Text>
            </Pressable>
          </Animated.View>
            ...
            {languages.map((lang) => (
              ...
              >
                <View style={styles.langInfo}>
                  <Text style={[styles.langNative, { color: theme.text, fontFamily: "Outfit_600SemiBold" }]}>
                    {lang.nativeName}
                  </Text>
                  <Text style={[styles.langName, { color: theme.textSecondary, fontFamily: "Outfit_400Regular" }]}>
                    {lang.name}
                  </Text>
                </View>
                {language === lang.code && (
                ...
                {lang.rtl && (
                  <View style={[styles.rtlBadge, { backgroundColor: theme.border }]}>
                    <Text style={[styles.rtlText, { color: theme.textSecondary, fontFamily: "Outfit_500Medium" }]}>RTL</Text>
                  </View>
                )}
              </Pressable>
            ))}
      ...
      <Modal visible={policyModal !== null} animationType="slide" transparent onRequestClose={() => setPolicyModal(null)}>
        <Pressable style={styles.modalOverlay} onPress={() => setPolicyModal(null)}>
          <Pressable style={[styles.modalSheet, { backgroundColor: theme.card }]} onPress={() => {}}>
            ...
            <Text style={[styles.policyText, { color: theme.textSecondary, fontFamily: "Outfit_400Regular" }]}>
              {policyModal?.content}
            </Text>
            <Pressable
              onPress={() => setPolicyModal(null)}
              style={({ pressed }) => [styles.policyCloseBtn, { backgroundColor: theme.accent, opacity: pressed ? 0.85 : 1 }]}
            >
              <Text style={[styles.policyCloseBtnText, { fontFamily: "Outfit_600SemiBold" }]}>{t("close")}</Text>
            </Pressable>
          </Pressable>
        </Pressable>
      </Modal>
...
const styles = StyleSheet.create({
  ...
  policyCloseBtn: { height: 50, borderRadius: 14, alignItems: "center", justifyContent: "center" },
  policyCloseBtnText: { color: "#fff", fontSize: 16 },
  logoutBtn: {
    height: 52,
    borderRadius: 14,
    flexDirection: "row",
